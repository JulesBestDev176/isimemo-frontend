...
import { mockRessourcesMediatheque } from '../../data/mock/ressources';
import { RessourceMediatheque, TypeCategorieRessource } from '../../types/ressource';
...
const Mediatheque: React.FC = () => {
  ...
  const [savedResourcesIds, setSavedResourcesIds] = useState<number[]>([]);
  ...
  const handleToggleSave = (idRessource: number) => {
    ...
    setSavedResourcesIds(prev => {
      if (prev.includes(idRessource)) {
        return prev.filter(id => id !== idRessource);
      } else {
        return [...prev, idRessource];
      }
    });
  };
  ...
  const isSaved = (idRessource: number): boolean => {
    return savedResourcesIds.includes(idRessource);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white border border-gray-200 p-6 mb-6 rounded-lg">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Médiathèque</h1>
            <p className="text-sm text-gray-600">
              Explorez toutes les ressources disponibles : cours, mémoires et canevas
            </p>
          </div>
        </div>
